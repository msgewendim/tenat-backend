...
export class UsersController {
  ...
  @Post(':id/cart')
  @UsePipes(new ValidationPipe())
  @ApiResponse({ status: 201, description: 'The item has been successfully added to the cart.', type: User })
  addToCart(@Param('id') id: string, @Body() cartItemDto: CartItemDto) {
    return this.usersService.addToCart(id, cartItemDto);
  }

  @Delete(':id/cart')
  @ApiResponse({ status: 200, description: 'The cart has been successfully cleared.', type: User })
