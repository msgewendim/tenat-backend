...
export class RecipesController {
  ...
  async findAll(
    ...
    const recipes = await this.recipesService.findAll(page, limit, searchTerm, category, excludeById);
    return { data: recipes };
  }

  @Get('names')
  @ApiResponse({ status: 200, description: 'Return recipes by name.', type: [Recipe] })
  getRecipesByName(@Query('name') name: string) {
    return this.recipesService.getRecipesByName(name);
  }

  @Get('random')
  @ApiResponse({ status: 200, description: 'Return random recipes.', type: [Recipe] })
  async getRandomRecipes(@Query('page') page: number, @Query('limit') limit: number) {
    const recipes = await this.recipesService.getRandomRecipes(page, limit);
    return { data: recipes };
  }

  @Get(':id')
  @ApiResponse({ status: 200, description: 'Return a single recipe.', type: Recipe })
